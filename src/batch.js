...
import {
  openDB,
  withTransaction,
  getAllAttestedRecordIds,
  upsertBatchHeader,
  insertBatchRecords,
  listRetryableBatchHeaders,
  markBatchSent,
  markBatchConfirmed,
  markBatchFailed,
  getBatchHeader,
  listPendingBatchHeaders,
  listUnconfirmedBatchHeaders,
  countBatchMembers,
} from "./tools/db.js";
...
async function recoverBatches(db) {
  ...
  const heads = await listRetryableBatchHeaders(db, {
    nowISO,
    minDelaySec: 30,
  });
      ...
      try {
        ...
        if (rcpt && rcpt.status === 1) {
          await markBatchConfirmed(db, {
            batch_id,
            attestation_uid: "unknown:recovered",
          });
...
async function runBatchCycle(db) {
  ...
  console.log(
    `🧺 New batch ${bid} | root=${root} | count=${newRecords.length}`,
  );
