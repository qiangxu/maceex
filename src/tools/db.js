...
export async function openDB(dbFile) {
  try {
    const dir = path.dirname(dbFile); // 从文件路径中获取目录路径
    await fs.mkdir(dir, { recursive: true }); // 递归创建目录，如果已存在则什么也不做
  } catch (err) {
    console.error("创建数据库目录失败:", err);
    throw err; // 如果创建目录失败，则抛出错误，终止后续操作
  }
...
export async function isProcessed(db, recordId) {
  const rows = await db.all(
    `SELECT record_id FROM attestations WHERE record_id NOT LIKE '__BATCH__:%'`,
  );
...
export async function upsertBatchHeader(
  db,
  { batch_id, merkle_root, proofs_cid },
) {
...
export async function markBatchConfirmed(
  db,
  { batch_id, attestation_uid, tx_hash },
) {
  const rid = `__BATCH__:${batch_id}`;
...
export async function insertBatchRecords(
  db,
  { batch_id, merkle_root, proofs_cid, record_ids },
) {
...
export async function countBatchMembers(db, batch_id) {
  const r = await db.get(
	...
	WHERE batch_id=? AND record_id NOT LIKE '__BATCH__:%'`,
    [batch_id],
  );
  return r.c || 0;
}

export async function listRetryableBatchHeaders(
  db,
  { nowISO, minDelaySec = 30 },
) {
